...
+-----------------------------------------------------------+
            ...
            <div>
                ...
                <div v-if="courseData.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div v-for="course in courseData" :key="course.id"
                ...
                <div v-if="courseDetails" @click="closeModalOnOutsideClick"
                    ...
                    <div class="bg-black modal-content p-6 border border-[#00b400] max-w-6xl w-full mx-4">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-xl font-bold text-[#00b400]">Course Details: {{ courseDetails.coursename }}</h3>
                            <button @click="courseDetails = null" class="text-[#00b400] hover:text-white">&times;</button>
                        </div>
                        <div class="overflow-auto max-h-[70vh] relative" id="courseDetailsContainer">
                            <div v-if="courseDetails.data" class="text-white">
                                <div class="mb-4 p-2 bg-[#001400] rounded-md">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        <button 
                                            v-for="(section, idx) in courseDetails.data" 
                                            :key="idx"
                                ...
                                >
                                    <span><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24"><path fill="currentColor" d="M13 19V7.83l4.88 4.88c.39.39 1.03.39 1.42 0a.996.996 0 0 0 0-1.41l-6.59-6.59a.996.996 0 0 0-1.41 0l-6.6 6.58a.996.996 0 1 0 1.41 1.41L11 7.83V19c0 .55.45 1 1 1s1-.45 1-1"/></svg></span>
                                </button>
                                
                                <div v-for="(section, sectionIndex) in courseDetails.data" :key="sectionIndex" class="mb-6" :id="section.kode_section">
                                    <h4 class="text-lg font-semibold text-[#00b400] mb-2">{{ section.nama_section }}</h4>
                                    <div v-for="(subSection, subIndex) in section.sub_section" :key="subIndex" class="ml-4 mb-2 p-2 hover:bg-[#001400] transition-colors border-l-2 border-[#00b400]"
                                        :class="{ 'hidden': !section.nama_section.includes('UTS') && 
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <span v-if="subSection.completion" class="ml-2 text-green-500">(Completed)</span>
                                        </div>
                                        <div v-if="subSection.konten" class="mt-1 text-sm text-gray-400" v-html="subSection.konten"></div>
                                        <div v-if="subSection.link" class="mt-1">
                                            <a :href="subSection.link" target="_blank" class="text-blue-400 hover:underline text-sm">Open Resource</a>
                                        </div>
                            ...
                            <div v-if="courseDetails.peserta && courseDetails.peserta.length > 0" class="mt-6">
                                <h4 class="text-lg font-semibold text-[#00b400] mb-2">Participants ({{ courseDetails.peserta.length }})</h4>
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                                    <div v-for="(student, index) in courseDetails.peserta" :key="index" class="text-sm p-2 border border-gray-800">
